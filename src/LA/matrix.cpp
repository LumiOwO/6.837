...
Matrix Matrix::Inverse(float epsilon) const {
  Matrix m = *this;
...
Matrix Matrix::MakeXRotation(float theta) {
  Matrix rx;
  rx.SetToIdentity();
  rx.data[1][1]= (float)cosf((float)theta);
  rx.data[1][2]=-(float)sinf((float)theta);
  rx.data[2][1]= (float)sinf((float)theta);
  rx.data[2][2]= (float)cosf((float)theta);
  return rx;
}
...
Matrix Matrix::MakeYRotation(float theta) {
  Matrix ry;
  ry.SetToIdentity();
  ry.data[0][0]= (float)cosf((float)theta);
  ry.data[0][2]= (float)sinf((float)theta);
  ry.data[2][0]=-(float)sinf((float)theta);
  ry.data[2][2]= (float)cosf((float)theta);
  return ry;
}
...
Vec4f Matrix::Transform(const Vec4f &v) const {
  Vec4f answer;
  for (int y=0; y<4; y++) {
    answer.data[y] = 0;
    for (int i=0; i<4; i++) {
...
Bound Matrix::operator()(const Bound &b) const
{
	const Matrix M = *this;
	return Bound(M(Vec3f(b.getMin().x(), b.getMin().y(), b.getMin().z())))
		.Extend(M(Vec3f(b.getMax().x(), b.getMin().y(), b.getMin().z())))
		.Extend(M(Vec3f(b.getMin().x(), b.getMax().y(), b.getMin().z())))
		.Extend(M(Vec3f(b.getMin().x(), b.getMin().y(), b.getMax().z())))
		.Extend(M(Vec3f(b.getMin().x(), b.getMax().y(), b.getMax().z())))
		.Extend(M(Vec3f(b.getMax().x(), b.getMax().y(), b.getMin().z())))
		.Extend(M(Vec3f(b.getMax().x(), b.getMin().y(), b.getMax().z())))
		.Extend(M(Vec3f(b.getMax().x(), b.getMax().y(), b.getMax().z())));
}

Ray Matrix::operator()(const Ray &r) const
{
	Vec3f origin = Transform(r.getOrigin());
	Vec3f dir = TransformDirection(r.getDirection());
	return Ray(origin, dir);
}

Vec3f Matrix::TransformNormal(const Vec3f &n, const Matrix *inv_p) const
{
	Matrix inv = inv_p ? (*inv_p) : this->Inverse();
	return inv.Transpose().TransformDirection(n).Normalize();
}
