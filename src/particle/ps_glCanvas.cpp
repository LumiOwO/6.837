...
void GLCanvas::initialize(Parser *_parser, float _refresh, float _dt, 
  ...
  // Ambient light
  GLfloat ambArr[] = { 0.1f,0.1f,0.1f,1.0f };
...
void GLCanvas::motion(int x, int y) {
  ...
  if (mouse_button == GLUT_LEFT_BUTTON) {
    float rx = 0.01f*(mouse_x - x);
    float ry = -0.01f*(mouse_y - y);
    // rotate around the center, keeping the y axis up
    Vec3f dir = camera_pos.Normalize();
    Vec3f up = Vec3f(0,1,0);
    Vec3f horiz = Vec3f::Cross3(dir,up).Normalize();
    float tiltAngle = acos(up.Dot3(dir));
    if (tiltAngle-ry > 3.13f)      ry = tiltAngle - 3.13f;
    else if (tiltAngle-ry < 0.01f) ry = tiltAngle - 0.01f;
    Matrix rotMat = Matrix::MakeAxisRotation(up, rx);
    rotMat *= Matrix::MakeAxisRotation(horiz, ry);
    dir = rotMat.TransformDirection(dir);
  ...
  // RIGHT BUTTON ZOOMS
    ...
    float exponent = 0.001f*(mouse_x - x);
    float zoom = (float)pow(10,exponent);
    camera_pos *= zoom;
  }
