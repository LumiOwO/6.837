...
int GLCanvas::theta_steps;
int GLCanvas::phi_steps;
bool GLCanvas::gouraud;
...
static int SPECULAR_FIX_WHICH_PASS = 0;
#endif
...
void GLCanvas::drawAxes(void)
{
  ...
  glBegin(GL_TRIANGLE_FAN);
	...
	glVertex3f(0.8f, 0.07f, 0.0f );
	glVertex3f(0.8f, 0.0f,  0.07f);
	glVertex3f(0.8f,-0.07f, 0.0f );
	glVertex3f(0.8f, 0.0f, -0.07f);
	glVertex3f(0.8f, 0.07f, 0.0f );
  glEnd();
  ...
  glBegin(GL_TRIANGLE_FAN);
	...
	glVertex3f( 0.07f, 0.0f,  0.8f);
	glVertex3f( 0.0f,  0.07f, 0.8f);
	glVertex3f(-0.07f, 0.0f,  0.8f);
	glVertex3f( 0.0f, -0.07f, 0.8f);
	glVertex3f( 0.07f, 0.0f,  0.8f);
  glEnd();
...
void GLCanvas::keyboard(unsigned char key, int i, int j) {
  ...
  case 't':  case 'T': {
	...
	float x = ((i + 0.5f) -  width/2.0f) / float(max) + 0.5f;
	float y = ((j + 0.5f) - height/2.0f) / float(max) + 0.5f;
    RayTree::Activate();
