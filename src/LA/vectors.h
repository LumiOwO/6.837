...
public:
  ...
  Vec3f& operator/=(float f) {
    ...
    data[2] /= f;
    return *this; }
  Vec3f operator-() const { return Vec3f(-data[0], -data[1], -data[2]); }
  ...
  Vec3f Permute(int x, int y, int z) const {
	  return Vec3f(data[x], data[y], data[z]);
  }
  ...
  friend Vec3f operator/(const Vec3f &v1, float f) {
	Vec3f v2 = v1; v2.Divide(f,f,f); return v2; }
  friend Vec3f operator/(const Vec3f &v1, int f) {
	Vec3f v2 = v1; v2.Divide((float)f,(float)f,(float)f); return v2;}
  ...
  static Vec3f Cross3(const Vec3f &v1, const Vec3f &v2) {
	  float x = v1.data[1] * v2.data[2] - v1.data[2] * v2.data[1];
	  float y = v1.data[2] * v2.data[0] - v1.data[0] * v2.data[2];
	  float z = v1.data[0] * v2.data[1] - v1.data[1] * v2.data[0];
	  return Vec3f(x, y, z);
